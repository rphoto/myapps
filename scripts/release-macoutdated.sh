...
# ---- collect release notes items (skipped when using a template file) ----
RELEASE_NOTES_LIS=""
if [ -z "$NOTES_TEMPLATE" ] || [ ! -f "$NOTES_TEMPLATE" ]; then
  echo ""
...
{
  printf '%s\n' '<!DOCTYPE html><html>'
  printf '%s' '<head><meta charset="UTF-8"><title>'
  printf '%s %s' "$APP_NAME" "$SHORT_VERSION"
  printf '%s\n' '</title></head><body>'
...
# If no "What's new" section found, try to extract all items
if [ -z "$lis" ]; then
lis="$(extract_lis "$f")"
fi
...
# ---- git: stage and commit all changed artifacts ----
echo ""
echo "• Staging files for git commit…"

# Resolve the repo root from REPO_DOCS (works even if run from any directory)
GIT_ROOT=$(git -C "$REPO_DOCS" rev-parse --show-toplevel 2>/dev/null) || {
  echo "⚠️  Warning: $REPO_DOCS is not inside a git repository — skipping git commit." >&2
  exit 0
}
